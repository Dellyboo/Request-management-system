...
// Get the redirect page from the session
if (isset($_SESSION['redirectPage'])) {
    $redirectPage = $_SESSION['redirectPage'];
    // Clear the session variable after getting the redirect page
    unset($_SESSION['redirectPage']);
} else {
    // Default page if no redirection info exists
...
<head>
    ...
    <script>
        ...
        setTimeout(function() {
            window.location.href = "<?php echo $redirectPage; ?>"; // Correct variable
        }, 3000);
    </script>
</head>
<body>

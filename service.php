...
try {
    $stmt = $pdo->prepare("SELECT names FROM employeenames WHERE id = 8");
    $stmt->execute();
...
$query = $pdo->prepare("
    ...
    WHERE poste_du_demandeur IN ('Chef Comptable')
");
$query->execute();
...
th {
    ...
    font-size: 1.35em; /* Increased font size for header */
...
<aside class="sidebar">
    ...
    <h2>Chef Financier et Comptable</h2>
    <ul>
...
$query = $pdo->prepare("SELECT id, nom_et_prenom, designation, quantite_demandee, status, requester_role FROM demandes WHERE requester_role IN ('Chef de Service', '.') ORDER BY requester_role ASC");
$query->execute();
        ...
        <div class="scrollable-table">
            <table>
                ...
                <tbody>
                    ...
                    $sql = "SELECT * FROM transportfee WHERE poste IN ('Chef Comptable')";
...
function confirmAction(action, id) {
    var message = (action === 'approve') 
        ? "Êtes-vous sûr de vouloir transmettre cette demande ?" 
    ...
    if (confirm(message)) {
        ...
        .then(data => {
            ...
            if (action === 'approve') {
                statusElement.textContent = 'en cours de vérification';
        ...
        <div class="scrollable-table">
            <table>
                ...
                <tbody>
                    ...
                    $sql = "SELECT * FROM fiche_de_demande_fonds WHERE poste_du_demandeur IN ('Chef Comptable')";
                    $result = $conn->query($sql);
                    ...
                    if ($result->num_rows > 0):
                        while ($row = $result->fetch_assoc()): ?>
                            <tr id="row-<?php echo $row['id']; ?>">
                                ...
                                ? 'status-approved' 
                                : ((!empty($row['status']) && $row['status'] === 'en attente (actuellement au DA&F)') 
                                ? 'status-pending' 
                                : 'status-undefined'));
                                ?>">
                                <?php echo htmlspecialchars($row['status'] ?? 'Non défini'); ?>
                        ...
                        <?php foreach ($decaissements as $decaissement): ?>
                            <tr id="row-decaissement-<?php echo $decaissement['id']; ?>">
                                ...
                                <td id="status-decaissement-<?php echo $decaissement['id']; ?>" 
                                    class="<?php echo ($decaissement['status'] === 'en cours de vérification (actuellement au DA&F)' || 
                                                       $decaissement['status'] === 'en cours de vérification (actuellement au DG)') 
                                           ? 'status-in-progress' : ''; ?>">
...
<script>
    function confirmAction(action, id) {
        const message = (action === 'approve') ? 
            'Êtes-vous sûr de vouloir transmettre cette demande vers le Directeur Administratif et Finacier pour la vérification suivante?' : 
            'Êtes-vous sûr de vouloir refuser cette demande ?';
        if (confirm(message)) {
            const xhr = new XMLHttpRequest();
            xhr.open('POST', 'decaissement/handle_decaissement.php', true);
...
<script>
    function UniqueconfirmAction(action, id) {
        ...
        if (confirm(message)) {
            const xhr = new XMLHttpRequest();
            xhr.open('POST', 'alimentation/handle_alimentation_caisse.php', true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    const response = JSON.parse(xhr.responseText);
            ...
            <tbody>
                <?php if (!empty($demandes)): ?>
                    <?php foreach ($demandes as $demande): ?>
                        <tr id="row-<?php echo $demande['id']; ?>">
                            <td><?php echo htmlspecialchars($demande['id']); ?></td>
                            <td><?php echo isset($demande['etabli_par']) ? htmlspecialchars($demande['etabli_par']) : 'N/A'; ?></td>
                            <td><?php echo isset($demande['beneficiaire']) ? htmlspecialchars($demande['beneficiaire']) : 'N/A'; ?></td>
                            <td><?php echo isset($demande['motif']) ? htmlspecialchars($demande['motif']) : 'N/A'; ?></td>
                            <td><?php echo isset($demande['montant']) ? htmlspecialchars($demande['montant']) : 'N/A'; ?></td>
                            <td><?php echo isset($demande['facture']) ? htmlspecialchars($demande['facture']) : 'N/A'; ?></td>
                            <td><?php echo isset($demande['mode']) ? htmlspecialchars($demande['mode']) : 'N/A'; ?></td>
                            <td id="status-<?php echo $demande['id']; ?>" 
                                class="<?php 
                                echo ($demande['status'] === 'déjà approuvé') 
                                    ? 'status-approved' 
                                    : (in_array($demande['status'], ['en cours de vérification (actuellement au DT&E)', 'en cours de vérification (actuellement au DA&F)', 'en cours de vérification (actuellement au DG)']) 
                                    ? 'status-in-progress' 
                                    : ''); 
                                ?>">
                                <?php echo isset($demande['status']) ? htmlspecialchars($demande['status']) : 'N/A'; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="7">Aucun statut de demande trouvé.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
.toggle-subtitle i {
    ...
    .divider {
        border-top: 1px solid rgb(32, 33, 33);
        margin-top: 15px;
        margin-left: 60px;
    }
...
$stmt = $pdo->prepare("SELECT * FROM poste WHERE id = 8");
$stmt->execute();
$userData = $stmt->fetch();
...
<div id="profile-popup" class="popup-form">
    <div class="popup-content">
        <h2><i class="bi bi-pencil-square-fill"></i> Modifier les informations personnelles</h2>
        <form id="profile-form">
            <div class="form-container">
                ...
                <div class="form-right">
                    ...
                    <div class="form-group">
                        <label for="naissance">
                            <i class="bi bi-calendar-date-fill"></i> Date de Naissance:
                        </label>
...
<div id="password-popup" class="popup-form">
    <div class="popup-content">
        <h2><i class=></i> Changer le mot de passe</h2>
        <form id="change-password-form" action="update-service.php" method="POST">
            <div class="form-group">
...
document.getElementById('change-password-form').addEventListener('submit', function(e) {
    ...
    fetch('passwords/update-service.php', {
        method: 'POST',
        body: formData
    })
...
<div id="form-content" class="content-section">
    ...
    <div class="icon-grid">
    <a href="consommation/form2.html" class="icon-item">
        <div class="icon-box">
            <i class="bi bi-clipboard-check-fill"></i>
            <p>Materiel de Consommation</p>
        </div>
    </a>
    ...
    <a href="autorisation/payement.php" class="icon-item">
        <div class="icon-box">
            <i class="bi bi-shield-fill-check"></i>
            <p>Autorisation de Payement</p>
        </div>
    </a>
    <div class="icon-grid">

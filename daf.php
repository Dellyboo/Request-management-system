...
try {
    $stmt = $pdo->prepare("SELECT names FROM employeenames WHERE id = 2");
    $stmt->execute();
...
$query = $pdo->prepare("
    ...
    FROM transportfee 
    WHERE poste IN ('Directeur administratif et financier', 'Chef Comptable', 
                     'Technicien de surface', 'Comptable', 'Caissière')");
$query->execute();
...
$query = $pdo->prepare("
    ...
    WHERE poste_du_demandeur IN ('Chef Comptable', 'Directeur administratif et financier', 
                                 'Technicien de surface', 'Comptable', 'Caissière')
");
$query->execute();
...
<aside class="sidebar">
    ...
    <h2>Dir. Administratif et financier</h2>
    <ul>
...
<div id="requests-content" class="content-section" style="overflow-y: auto; max-height: 90vh;">
    ...
    <!-- Subtitle 1 -->
    <h2 onclick="toggleSection('materielSection', 'iconMateriel')" 
        ...
        <span id="iconMateriel" class="bi bi-clipboard-check" style="margin-right: 10px;"></span>
    ...
    <!-- Divider -->
    <hr style="border: none; border-top: 1px solid #006400; margin: 5px 0; margin-left: 75px;">
...
<h2 onclick="toggleSection('ficheDemandeFondsSection', 'iconFiche')" 
    style="cursor: pointer; color: black; text-align: left; border-bottom: 1px solid transparent; padding-bottom: 3px; margin-bottom: 2px;">
    <span id="iconFiche" class="bi bi-file-earmark-text" style="margin-right: 10px;"></span>
...
$query = $pdo->prepare("SELECT id, nom_et_prenom, designation, quantite_demandee, status, requester_role FROM demandes WHERE requester_role IN ('Chef de Service', 'Chef de Projet', 'DAF', 'DTE') ORDER BY requester_role ASC");
$query->execute();
$demandes = $query->fetchAll(PDO::FETCH_ASSOC);
?>
                        ...
                        <?php if (!empty($demandes)): ?>
                            <?php foreach ($demandes as $demande): ?>
                                <tr id="row-<?php echo $demande['id']; ?>">
                                    ...
                                    <td><?php echo htmlspecialchars($demande['designation']); ?></td>
                                    <td><?php echo htmlspecialchars($demande['quantite_demandee']); ?></td>
                                    <td id="status-<?php echo $demande['id']; ?>" class="<?php echo ($demande['status'] === 'déjà approuvé') ? 'status-approved' : ($demande['status'] === 'en cours de vérification (actuellement au DG)' ? 'status-in-progress' : ''); ?>">
                                        <?php echo htmlspecialchars($demande['status']); ?>
                                    </td>
                                    <td>
                                        <?php if (in_array($demande['requester_role'], ['Chef de Service', 'Chef de Projet', 'DTE'])): ?>
        ...
        <div class="scrollable-table">
            <table>
                ...
                <tbody>
                    ...
                    $sql = "SELECT * FROM transportfee WHERE poste IN ('Directeur administratif et financier', 'Directeur technique et exploitation', 'Chef de Projet', 'Maintenancier des scanners', 'Suivi électronique', 'Chef Comptable', 'Technicien de surface', 'Comptable', 'Caissière')";
                            ...
                            <tr id="row-<?php echo $row['id']; ?>">
                                ...
                                <td id="status-<?php echo $row['id']; ?>" class="<?php 
                                    echo (!empty($row['status']) && $row['status'] === 'déjà approuvé') ? 'status-approved' : 
                                         ((!empty($row['status']) && $row['status'] === 'en cours de vérification (actuellement au DG)') ? 'status-in-progress' : '' );
                                ?>">
                                    <?php echo htmlspecialchars($row['status'] ?? 'Non défini'); ?>
                                </td>
                                <td>
                                    <!-- Approve Button -->
                                    <button onclick="UconfirmAction('approve', <?php echo $row['id']; ?>)" class="action-button approve-btn">Transmettre</button>

                                    <!-- Reject Button -->
                                    <button onclick="UconfirmAction('delete', <?php echo $row['id']; ?>)" class="action-button delete-btn">Refuser</button>
                                </td>
                            </tr>
                        <?php endwhile;
                    else: ?>
...
<script>
function UconfirmAction(action, id) {
    var message = (action === 'approve') 
        ? "Êtes-vous sûr de vouloir transmettre cette demande vers le Directeur Général pour l'accord?" 
    ...
    if (confirm(message)) {
        ...
        .then(data => {
            ...
            if (action === 'approve') {
                statusElement.textContent = 'en cours de vérification (actuellement au DG)';
        ...
        <div class="scrollable-table">
            <table>
                ...
                <tbody>
                    ...
                    $sql = "SELECT * FROM fiche_de_demande_fonds WHERE poste_du_demandeur IN ('Directeur administratif et financier', 'Chef Comptable', 'Technicien de surface', 'Comptable', 'Caissière')";
                    $result = $conn->query($sql);
                    ...
                    if ($result->num_rows > 0):
                        while ($row = $result->fetch_assoc()): ?>
                            <tr id="row-<?php echo $row['id']; ?>">
                                ...
                                <td><?php echo htmlspecialchars($row['montant_en_lettre']); ?></td>
                                <td><?php echo htmlspecialchars($row['created_at']); ?></td>
                                <td id="status-<?php echo htmlspecialchars($row['id']); ?>" 
                                class="<?php 
                                echo (!empty($row['status']) && $row['status'] === 'en cours de vérification (actuellement au DG)') 
                                ? 'status-in-progress' 
                                : ((!empty($row['status']) && $row['status'] === 'déjà approuvé') 
                                ? 'status-approved' 
                                : ((!empty($row['status']) && $row['status'] === 'en attente') 
                                ? 'status-pending' 
                                : 'status-undefined'));
                                ?>">
                                <?php echo htmlspecialchars($row['status'] ?? 'Non défini'); ?>
                                ...
                                <td>
                                    <!-- Approve Button -->
                                    <button onclick="AconfirmActionFiche('approve', <?php echo $row['id']; ?>)" class="action-button approve-btn">Transmettre</button>

                                    <!-- Reject Button -->
                                    <button onclick="AconfirmActionFiche('delete', <?php echo $row['id']; ?>)" class="action-button delete-btn">Refuser</button>
                                </td>
                            </tr>
                        <?php endwhile;
                    else: ?>
...
<script>
    ...
    function handleAction(action, id) {
        // Set the action and ID to be used in the modal
        currentAction = action;
        currentId = id;

        const modalMessage = document.getElementById('modal-message');
        if (action === 'approve') {
            modalMessage.innerText = 'Êtes-vous sûr de vouloir transmettre cette demande vers le Directeur Général pour l\'accord?';
        } else if (action === 'delete') {
            modalMessage.innerText = 'Êtes-vous sûr de vouloir refuser cette demande?';
        }
    ...
    document.getElementById('confirm-action').addEventListener('click', () => {
        if (currentAction === 'approve') {
            ...
            fetch('update_status.php', {
                ...
                body: JSON.stringify({ id: currentId, status: 'en cours de vérification (actuellement au DG)' }),
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    document.getElementById(`status-${currentId}`).innerText = 'en cours de vérification (actuellement au DG)';
                    document.getElementById(`status-${currentId}`).classList.remove('status-approved');
                    document.getElementById(`status-${currentId}`).classList.add('status-in-progress');
            ...
            // Send request to delete item from the database
            ...
            .then(data => {
                if (data.success) {
                    document.getElementById(`row-${currentId}`).remove();
...
<script>
function AconfirmActionFiche(action, id) {
    var message = (action === 'approve') 
        ? "Êtes-vous sûr de vouloir transmettre cette demande vers le Directeur Général pour l'accord?" 
        : "Êtes-vous sûr de vouloir refuser cette demande ?";
...
// Fetch data from the decaissement table
$query = "SELECT * FROM decaissement";
$stmt = $pdo->prepare($query);
$stmt->execute();
$decaissements = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
<!-- Subtitle "Pour le Décaissement" -->
<div class="section">
    ...
    <div id="decaissement" class="content" style="display: none;">
        <div class="confirmation-message" id="confirmation-message-decaissement"></div>
        <div class="scrollable-table">
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php if (!empty($decaissements)): ?>
                        <?php foreach ($decaissements as $decaissement): ?>
                            <tr id="row-decaissement-<?php echo $decaissement['id']; ?>">
                                ...
                                <td id="status-decaissement-<?php echo $decaissement['id']; ?>" 
                                    class="<?php echo ($decaissement['status'] === 'en cours de vérification (actuellement au DG)') 
                                        ? 'status-in-progress' : ''; ?>">
                                    <?php echo htmlspecialchars($decaissement['status']); ?>
                                </td>
                                <td>
                                    <button class="action-button approve-btn" 
                                            onclick="confirmAction('approve', <?php echo $decaissement['id']; ?>)">Transmettre</button>
                                    <button class="action-button delete-btn" 
                                            onclick="confirmAction('delete', <?php echo $decaissement['id']; ?>)">Refuser</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="12">Aucune demande de décaissement trouvée.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<script>
    function confirmAction(action, id) {
        ...
        if (confirm(message)) {
            const xhr = new XMLHttpRequest();
            xhr.open('POST', 'decaissement/handle_2.php', true);
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    ...
                    if (response.success) {
                        if (action === 'approve') {
                            document.getElementById('status-decaissement-' + id).innerText = 
                                'en cours de vérification (actuellement au DA&F)';
                        } else if (action === 'delete') {
                            document.getElementById('row-decaissement-' + id).remove();
                        }
...
// Fetch data from the "alimentation de caisse" table
$query = "SELECT * FROM `alimentation de caisse`";
$stmt = $pdo->prepare($query);
$stmt->execute();
...
<div class="section">
    <h2 class="toggle-subtitle" onclick="toggleContent('alimentation-caisse')">
        <i class="bi bi-file-earmark-plus-fill"></i> Pour l'alimentation de caisse
        <span class="toggle-icon">+</span> <!-- + sign initially -->
    </h2>
    <div id="alimentation-caisse" class="content" style="display: none;">
        <div class="confirmation-message" id="confirmation-message-alimentation"></div>
        <div class="scrollable-table">
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php if (!empty($alimentations)): ?>
                        <?php foreach ($alimentations as $alimentation): ?>
                            <tr id="row-alimentation-<?php echo $alimentation['id']; ?>">
                                <td><?php echo htmlspecialchars($alimentation['id']); ?></td>
                                <td><?php echo isset($alimentation['demandeur']) ? htmlspecialchars($alimentation['demandeur']) : 'N/A'; ?></td>
                                <td><?php echo isset($alimentation['montant']) ? htmlspecialchars($alimentation['montant']) : 'N/A'; ?></td>
                                <td><?php echo isset($alimentation['total_lettres']) ? htmlspecialchars($alimentation['total_lettres']) : 'N/A'; ?></td>
                                <td><?php echo isset($alimentation['motif']) ? htmlspecialchars($alimentation['motif']) : 'N/A'; ?></td>
                                <td><?php echo isset($alimentation['date']) ? htmlspecialchars($alimentation['date']) : 'N/A'; ?></td>
                                <td id="status-alimentation-<?php echo $alimentation['id']; ?>" 
                                    class="<?php 
                                    echo ($alimentation['status'] === 'déjà approuvé') 
                                        ? 'status-approved' 
                                        : (in_array($alimentation['status'], ['en cours de vérification (actuellement au DG)']) 
                                        ? 'status-in-progress' 
                                        : ''); 
                                    ?>">
                                    <?php echo isset($alimentation['status']) ? htmlspecialchars($alimentation['status']) : 'N/A'; ?>
                                </td>
                                <td>
                                    <button class="action-button approve-btn" 
                                            onclick="UniqueconfirmAction('approve', <?php echo $alimentation['id']; ?>)">Transmettre</button>
                                    <button class="action-button delete-btn" 
                                            onclick="UniqueconfirmAction('delete', <?php echo $alimentation['id']; ?>)">Refuser</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="9">Aucune demande d'alimentation de caisse trouvée.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<script>
    function UniqueconfirmAction(action, id) {
        const message = (action === 'approve') ? 
            'Êtes-vous sûr de vouloir transmettre cette demande vers le Directeur Général pour l\'accord?' : 
            'Êtes-vous sûr de vouloir refuser cette demande?';
        if (confirm(message)) {
            const xhr = new XMLHttpRequest();
            xhr.open('POST', 'alimentation/handle_2.php', true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    const response = JSON.parse(xhr.responseText);
                    if (response.success) {
                        if (action === 'approve') {
                            document.getElementById('status-alimentation-' + id).innerText = 
                                'en cours de vérification (actuellement au DA&F)';
                        } else if (action === 'delete') {
                            document.getElementById('row-alimentation-' + id).remove();
                        }
                        alert(response.message);
                    } else {
                        alert('Une erreur est survenue. Veuillez réessayer.');
...
// Fetch data from paymentgrant table
$sql = "SELECT id, etabli_par, beneficiaire, motif, montant, facture, status, mode FROM paymentgrant";
$result = $conn->query($sql);

// Check if there are records
if ($result->num_rows > 0) {
    $demandes = $result->fetch_all(MYSQLI_ASSOC); // Fetch all records as associative array
} else {
    $demandes = []; // No records found
}
...
<div class="section">
    <h2 class="toggle-subtitle" onclick="toggleContent('autorisation')">
        <i class="bi bi-shield-fill-check"></i> Pour l'autorisation de payement
                ...
                <tbody>
                    <?php if (!empty($demandes)): ?>
                        <?php foreach ($demandes as $demande): ?>
                            <tr id="row-<?php echo $demande['id']; ?>">
                                <td><?php echo htmlspecialchars($demande['id']); ?></td>
                                <td><?php echo htmlspecialchars($demande['etabli_par']); ?></td>
                                ...
                                <td><?php echo htmlspecialchars($demande['facture']); ?></td>
                                <td><?php echo htmlspecialchars($demande['mode']); ?></td>
                                <td id="status-<?php echo $demande['id']; ?>" class="<?php echo ($demande['status'] === 'déjà approuvé') ? 'status-approved' : ($demande['status'] === 'en cours de vérification (actuellement au DG)' ? 'status-in-progress' : ''); ?>">
                                    <?php echo htmlspecialchars($demande['status']); ?>
                                </td>
                                <td>
                                    <button class="action-button approve-btn" onclick="confirmAndUpdateStatus(<?php echo $demande['id']; ?>)">Transmettre</button>
                                    <button class="action-button delete-btn" onclick="confirmAndDeleteEntry(<?php echo $demande['id']; ?>)">Refuser</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="8">Aucun statut de demande trouvé.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<!-- JavaScript -->
<script>
    function confirmAndUpdateStatus(id) {
        const confirmation = window.confirm('Êtes-vous sûr de vouloir transmettre cette demande vers le Directeur Général pour l\'accord?');
        if (confirmation) {
            fetch('autorisation/update-status.php', {
                method: 'POST',
            ...
            .then(data => {
                if (data.success) {
                    document.getElementById(`status-${id}`).textContent = 'en cours de vérification (actuellement au DG)';
                    document.getElementById(`status-${id}`).className = 'status-in-progress';
                    alert('Demande transmise avec succès !');
                } else {
                    alert('Échec de la transmission du statut.');
                }
            })
            .catch(error => {
                console.error('Error:', error);
...
// Fetch data from the 'poste' table where id = 2
$stmt = $pdo->prepare("SELECT * FROM poste WHERE id = 2");
$stmt->execute();
$userData = $stmt->fetch();
...
// Proceed with using $userData (e.g., display user info)
?>
    ...
    <table class="employee-table">
        ...
        <tbody>
            ...
            try {
                ...
                $query = "SELECT * FROM poste WHERE poste IN ('Directeur administratif et financier', 'Chef Comptable', 'Technicien de surface', 'Comptable', 'Caissière', 'Chef de Service')";
...
<div id="form-content" class="content-section">
    ...
    <div class="icon-grid">
    <a href="consommation/form3.html" class="icon-item">
        <div class="icon-box">
...
function handleAction(action, id) {
    if (action === 'approve') {
        if (confirm('Êtes-vous sûr de vouloir transmettre cette demande vers le Directeur Général pour l,accord?')) {
            ...
            fetch('update_status.php', {
                ...
                body: JSON.stringify({ id: id, status: 'en cours de vérification (actuellement au DG)' }),
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
